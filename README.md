...
| **Inter-session messaging** | Every session's shell gets the bundled `termhub-msg` CLI on its `PATH` (with `TERMHUB_SESSION_ID` / `TERMHUB_SESSION_NAME` in its env): `termhub-msg list` shows the other open sessions, `termhub-msg send <name> <text…>` messages one, `termhub-msg inbox` reads what's waiting — so one agent can hand a task to another without leaving the terminal. A button in the sidebar toggles a panel on the right edge with the live message log. macOS/Unix only. |
